...
LABEL_PROPOSALS_PATH = ROOT / "data" / "reports" / "semantic-map-hierarchy-label-proposals-v1.json"
...
def build_payload(include_label_proposals: bool = False) -> dict[str, Any]:
    ...
    label_report = None
    if include_label_proposals:
        label_report = read_json(LABEL_PROPOSALS_PATH)
        if (
            label_report.get("status") != "human-review-required"
            or label_report.get("proposalOnly") is not True
            or label_report.get("corpusDigest") != hierarchy["corpus"]["corpusDigest"]
            or len(label_report.get("fine") or {}) != 32
            or len(label_report.get("top") or {}) != 8
        ):
            raise ValueError("Hierarchy label proposals are incomplete or stale")
    variants = []
    for variant in hierarchy["variants"]:
        ...
        variant_labels = None
        if label_report and label_report["blindVariantId"] == blind_id:
            variant_labels = {
                level: {
                    cluster_id: item["response"]
                    for cluster_id, item in label_report[level].items()
                }
    ...
    return {
        ...
        "labelReview": include_label_proposals,
    }
...
#map{width:100%;height:100vh;display:block;touch-action:none;cursor:grab}#map.dragging{cursor:grabbing}.point{stroke:#0a0c11;stroke-width:.7;vector-effect:non-scaling-stroke;cursor:pointer}.point.dim{opacity:.07}.point.match{stroke:#fff;stroke-width:2.2}.relation{stroke:#fff;stroke-width:1.2;stroke-dasharray:4 3;opacity:.8;vector-effect:non-scaling-stroke;pointer-events:none}.map-label{fill:#fff;stroke:#080a0e;stroke-width:3.5;paint-order:stroke;stroke-linejoin:round;font-weight:800;font-size:13px;text-anchor:middle;pointer-events:none}.map-label.dim{opacity:.12}
...
<main class="app">
 <section class="stage" aria-label="Interaktywna mapa porównawcza">
  ...
  <svg id="map" role="img" aria-label="914 gier na projekcji UMAP"><g id="viewport"><g id="relations"></g><g id="points"></g><g id="labels"></g></g></svg>
...
const svg=document.getElementById('map'), viewport=document.getElementById('viewport'), pointLayer=document.getElementById('points'), relationLayer=document.getElementById('relations'), labelLayer=document.getElementById('labels');
...
function labelFor(a){const labels=variant().labels;if(!labels)return null;return {fine:labels.fine[a.fine],top:labels.top[a.top]}}
function transform(){viewport.setAttribute('transform',`translate(${panX} ${panY}) scale(${scale})`)}
function setTooltip(event,p){const a=p.assignments[variantId],labels=labelFor(a);const clusterText=labels?`${labels.top.namePl} / ${labels.fine.namePl}`:`${a.top} / ${a.fine}`;tooltip.innerHTML=`<strong>${escapeHtml(p.title)}</strong><div class="meta">${p.id} · ${escapeHtml(p.author||'autor nieustalony')} · ${p.year||'rok nieustalony'}<br>${escapeHtml(p.source)}<br>${escapeHtml(clusterText)}</div><p>${escapeHtml(p.summary||'Brak krótkiego streszczenia.')}</p>`;tooltip.style.display='block';moveTooltip(event)}
...
function renderPoints(){pointLayer.replaceChildren();relationLayer.replaceChildren();const q=search.value.trim().toLocaleLowerCase('pl');const parentValue=parent.value;for(const p of data.points){const a=p.assignments[variantId],labels=labelFor(a);const circle=document.createElementNS('http://www.w3.org/2000/svg','circle');circle.setAttribute('cx',p.pos.x);circle.setAttribute('cy',p.pos.y);circle.setAttribute('r',selectedCluster===a.fine?5:3.15);circle.setAttribute('fill',color(a));circle.classList.add('point');const searchable=`${p.id} ${p.title} ${p.author} ${p.source} ${labels?labels.fine.namePl+' '+labels.fine.nameEn+' '+labels.top.namePl+' '+labels.top.nameEn:''}`;const matches=!q||searchable.toLocaleLowerCase('pl').includes(q);const visible=matches&&(!selectedCluster||a.fine===selectedCluster)&&(!parentValue||a.top===parentValue);if(!visible)circle.classList.add('dim');if(q&&matches)circle.classList.add('match');circle.addEventListener('pointerenter',e=>setTooltip(e,p));circle.addEventListener('pointermove',moveTooltip);circle.addEventListener('pointerleave',()=>tooltip.style.display='none');circle.addEventListener('click',()=>window.open(p.url,'_blank','noopener'));pointLayer.append(circle)}
...
function renderLabels(){labelLayer.replaceChildren();const v=variant();if(!v.labels)return;const parentValue=parent.value;for(const [topId,label] of Object.entries(v.labels.top)){const members=data.points.filter(p=>p.assignments[variantId].top===topId);const x=members.reduce((s,p)=>s+p.pos.x,0)/members.length,y=members.reduce((s,p)=>s+p.pos.y,0)/members.length;const text=document.createElementNS('http://www.w3.org/2000/svg','text');text.setAttribute('x',x);text.setAttribute('y',y);text.classList.add('map-label');if(parentValue&&parentValue!==topId)text.classList.add('dim');text.textContent=label.namePl.length>42?label.namePl.slice(0,41)+'…':label.namePl;labelLayer.append(text)}}
function metric(label,value){return `<div class="metric"><span>${label}</span><b>${value}</b></div>`}
function renderSidebar(){const v=variant();document.getElementById('variant-title').textContent=`Wariant ${variantId.replace('candidate-','')}${v.labels?' · propozycje nazw':''}`;const gate=document.getElementById('gate');gate.className=v.eligible?'ok':'warning';gate.textContent=v.labels?'Nazwy LLM oczekują na zatwierdzenie człowieka.':(v.eligible?'Spełnia bramkę minimalnego rozmiaru.':'Uwaga: zawiera klaster mniejszy niż 5 gier.');const m=v.metrics;document.getElementById('metrics').innerHTML=metric('semantic silhouette',m.semanticSilhouette)+metric('visual silhouette',m.visualSilhouette)+metric('najmniejszy klaster',m.minimumFineClusterSize)+metric('minimalne ARI',m.minimumAdjustedRandIndex);
const parents=[...new Set(v.clusters.map(c=>c.parentId))].sort();const old=parent.value;parent.innerHTML='<option value="">Wszystkie regiony</option>'+parents.map(id=>`<option value="${id}">${v.labels?escapeHtml(v.labels.top[id].namePl):id}</option>`).join('');parent.value=parents.includes(old)?old:'';const wrap=document.getElementById('clusters');wrap.replaceChildren();for(const c of v.clusters){const div=document.createElement('div');div.className='cluster'+(selectedCluster===c.id?' active':'');const a={fine:c.id,top:c.parentId},label=v.labels?v.labels.fine[c.id]:null,parentLabel=v.labels?v.labels.top[c.parentId]:null;div.innerHTML=`<div class="cluster-head"><span class="swatch" style="background:${color(a)}"></span><b>${label?escapeHtml(label.namePl):c.id}</b><small>${parentLabel?escapeHtml(parentLabel.namePl):c.parentId} · ${c.size} gier</small></div>${label?`<p>${escapeHtml(label.descriptionPl)}</p>`:''}<ul>${c.central.map(p=>`<li>${escapeHtml(p.title)} <small>(${p.id})</small></li>`).join('')}</ul>`;div.addEventListener('click',()=>{selectedCluster=selectedCluster===c.id?'':c.id;renderSidebar();renderPoints();renderLabels()});wrap.append(div)}}
document.querySelectorAll('[data-variant]').forEach(button=>button.addEventListener('click',()=>{variantId=button.dataset.variant;selectedCluster='';document.querySelectorAll('[data-variant]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));renderSidebar();renderPoints();renderLabels()}));
search.addEventListener('input',renderPoints);parent.addEventListener('change',()=>{renderPoints();renderLabels()});document.getElementById('reset').addEventListener('click',()=>{scale=1;panX=panY=0;selectedCluster='';search.value='';parent.value='';transform();renderSidebar();renderPoints();renderLabels()});
...
document.getElementById('questions').innerHTML=data.expertQuestions.map(q=>`<li>${escapeHtml(q)}</li>`).join('');renderSidebar();renderPoints();renderLabels();
</script>
</body></html>
"""
...
def main() -> None:
    ...
    parser.add_argument("--include-label-proposals", action="store_true")
    args = parser.parse_args()
    payload = build_payload(args.include_label_proposals)
